...
export const ACTION_TYPES = {
  SEARCH_RECIPIENTS: 'recipient/SEARCH_RECIPIENTS',
...
export default (state: RecipientState = initialState, action): RecipientState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_RECIPIENTS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_RECIPIENTS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_RECIPIENTS):
...
const apiSearchUrl = 'api/_search/recipients';

// Actions

export const getSearchEntities: ICrudSearchAction<IRecipient> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_RECIPIENTS,
  payload: axios.get<IRecipient>(`${apiSearchUrl}?query=${query}`)
});
